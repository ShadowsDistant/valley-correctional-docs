...
// Seniors (tier >= 3) and admins can approve/reject pending logs in that division.
function canApprove(user, div) {
  if (!user || isSuspended(user)) return false;
  if (user.role === 'admin') return true;
  if (div) return rankTier(user, div) >= 3;
  return ['moderation', 'sid'].some((d) => rankTier(user, d) >= 3);
}

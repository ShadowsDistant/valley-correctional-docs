...
function findUserByUsername(username) {
  // Archived (soft-deleted) accounts can never sign in.
  return db
    .prepare('SELECT * FROM users WHERE lower(username) = lower(?) AND deleted = 0')
    .get(username);
}
...
// Management or the Oversight Committee (covers facility administration leadership).
...
// Community feedback triage uses the same circle: Management/Oversight (any rank) + admins.
const canFeedbackStaff = canManageShifts;
...
const RANKS = {
  ...
  prison_mgr:     { label: 'Facility Administration Manager',           division: 'management', tier: 4, authority: 85, governs: ['management'] },
  asst_prison_mgr:{ label: 'Assistant Facility Administration Manager', division: 'management', tier: 4, authority: 80, governs: ['management'] },
...
// Voiding is restricted to specific ranks (handbook policy) plus admins.
// Punishments: Senior Moderator, IOM, Asst IOM, Asst CM, CM, Lead Overseer.
// Infractions: Lead Investigator, Community Manager, Lead Overseer.
const VOID_PUN_RANKS = ['sr_mod', 'iom', 'asst_iom', 'asst_community_mgr', 'community_mgr', 'lead_over'];
const VOID_INF_RANKS = ['lead_inv', 'community_mgr', 'lead_over'];
function hasAnyRank(user, keys) {
  const held = Object.values(userRanks(user || {}));
  return keys.some((k) => held.includes(k));
}
function canVoidPunishment(user) {
  return !!user && !isSuspended(user) && (user.role === 'admin' || hasAnyRank(user, VOID_PUN_RANKS));
}
function canVoidInfraction(user) {
  return !!user && !isSuspended(user) && (user.role === 'admin' || hasAnyRank(user, VOID_INF_RANKS));
}
...
function verifyCsrf(req, res, next) {
  ...
  // A stale login form (e.g. restored from cache after logout) just needs a
  // fresh one — bounce back to the form instead of a dead-end error page.

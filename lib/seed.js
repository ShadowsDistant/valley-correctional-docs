...
function runPageMigrations() {
  ...
  // Drop the "Official Community Hub" hero eyebrow from the home page. This is a
  // surgical removal of one element rather than a whole-body refresh, so it also
  // applies to a home page an admin has edited — their prose is left untouched.
  if (!migrationDone('migrate.home-eyebrow')) {
    const row = db.prepare("SELECT content FROM pages WHERE slug = 'home'").get();
    if (row && /hero-eyebrow/.test(row.content)) {
      const next = row.content
        .replace(/<div class="hero">\s*<span class="hero-eyebrow">[\s\S]*?<\/span>\s*<\/div>\s*/i, '')
        .replace(/^\s*<span class="hero-eyebrow">[\s\S]*?<\/span>\s*/i, '');
      if (next !== row.content) {
        db.prepare("UPDATE pages SET content = ? WHERE slug = 'home'").run(next);
        console.log('[migrate] removed the home hero eyebrow');
      }
    }
    markMigration('migrate.home-eyebrow');
  }
}
...
// City names captured before the header-encoding fix were stored as mojibake
// ("Goiás" -> "GoiÃ¡s"), because Node hands header values back latin1-decoded.
// Re-read those rows' bytes as UTF-8. Only rows that decode cleanly are touched.
function repairViewCities() {
  if (migrationDone('migrate.city-encoding')) return;
  let rows = [];
  try {
    rows = db.prepare("SELECT DISTINCT city FROM page_views WHERE city != '' AND city IS NOT NULL").all();
  } catch (e) { return; }              // column may not exist on an old DB yet
  const upd = db.prepare('UPDATE page_views SET city = ? WHERE city = ?');
  let n = 0;
  for (const { city } of rows) {
    if (!/[\u0080-\u00ff]/.test(city)) continue;
    const fixed = Buffer.from(city, 'latin1').toString('utf8');
    if (fixed === city || fixed.includes('�')) continue;
    upd.run(fixed, city);
    n++;
  }
  if (n) console.log(`[migrate] repaired encoding on ${n} city name(s)`);
  markMigration('migrate.city-encoding');
}

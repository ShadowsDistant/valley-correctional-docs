...
function refreshPagesFromFiles(slugs, why) {
  const upd = db.prepare(
    "UPDATE pages SET content=@content, updated_at=datetime('now'), updated_by='system' WHERE slug=@slug"
  );
  let n = 0;
  for (const slug of slugs) {
    const man = PAGES.find((p) => p.slug === slug);
    if (man) n += upd.run({ slug, content: readContent(man.file) }).changes;
  }
  if (n) console.log(`[migrate] refreshed ${n} page(s): ${why}`);
}
function runPageMigrations() {
  ...
  if (ren) {
    ...
    db.prepare("UPDATE pages SET content=@content, description=@description WHERE slug='shifts/shift-schedule'")

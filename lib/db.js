...
// ISO-3166 country from Cloudflare's CF-IPCountry header (for the traffic globe).
addColumn('page_views', 'country', "country TEXT DEFAULT ''");
// City-level origin from Cloudflare's visitor-location Managed Transform headers
// (CF-IPCity / CF-IPLatitude / CF-IPLongitude) so the globe can plot exact spots.
addColumn('page_views', 'city', "city TEXT DEFAULT ''");
addColumn('page_views', 'lat', 'lat REAL');
addColumn('page_views', 'lon', 'lon REAL');
db.exec('CREATE INDEX IF NOT EXISTS idx_views_user ON page_views(username, ts)');

// Deployed-build history for Admin → System. One row per commit this instance has
// ever booted on, recorded at startup by comparing data/build.json to the newest
// row — so it captures updates run from the button and manual host deploys alike.
...
db.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_update_commit ON update_log(commit_sha)');
...
db.exec(`
  ...
  -- VCF Assistant conversations. Stored on this server (never sent anywhere
  -- but the model provider at ask time) so staff keep their history and
  -- admins can review usage. Reasoning is kept alongside each answer.
  ...
  CREATE TABLE IF NOT EXISTS ai_messages (
    ...
    thinking   TEXT NOT NULL DEFAULT '', -- model reasoning (assistant turns)
    created_at TEXT NOT NULL DEFAULT (datetime('now')),
  ...
  -- WebAuthn passkeys used as a second factor at login. We store the SPKI
  -- public key (from the browser's getPublicKey()) — never any secret; the
  -- private key never leaves the user's device.
  CREATE TABLE IF NOT EXISTS passkeys (
    id          INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id     INTEGER NOT NULL,
    cred_id     TEXT NOT NULL UNIQUE,     -- base64url credential id
    public_key  TEXT NOT NULL,            -- base64url SPKI DER
    alg         INTEGER NOT NULL,         -- COSE alg id (-7 ES256, -257 RS256)
...
`);
// Feedback category (idea/bug/improvement/other) — drives the filter chips.
addColumn('feedback', 'category', "category TEXT NOT NULL DEFAULT 'idea'");
// Reason staff gave when approving/rejecting; shown on the card.
addColumn('feedback', 'status_reason', "status_reason TEXT DEFAULT ''");
// Soft-deleted chat messages: kept + shown struck to staff, hidden from submitters.
addColumn('feedback_messages', 'deleted', 'deleted INTEGER NOT NULL DEFAULT 0');
addColumn('feedback_messages', 'deleted_by', 'deleted_by TEXT');
